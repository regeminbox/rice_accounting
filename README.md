...
### 알림창을 닫으면 키보드 입력이 되지 않던 문제

**증상.** 저장 완료 알림을 닫은 뒤 입력란을 클릭해도 글자가 입력되지 않고,
다른 창을 눌렀다가 돌아와야 다시 입력이 가능했습니다.

**원인.** Windows Electron에서 웹 표준 `alert()` / `confirm()`이 닫힐 때
웹 콘텐츠 영역이 키보드 포커스를 잃는 문제였습니다.
해당 호출은 앱 전체 70여 곳에 흩어져 있었습니다.

**해결.** 호출부를 하나씩 고치면 이후 추가되는 화면에서 같은 문제가 반복되므로,
호출 방식 자체를 교체했습니다.

- `electron.js` — 메인 프로세스에서 `dialog.showMessageBoxSync`로 대화상자를 띄우고,
  닫힌 직후 `blur()` → `focus()` → `webContents.focus()`로 포커스를 되돌립니다.
- `index.tsx` — 렌더러 진입 시점에 `window.alert` / `window.confirm`을
  위 IPC 호출로 **전역 교체**합니다.

그 결과 기존 70여 곳이 한 번에 해결되었고, 이후 작성되는 코드에서
`alert()`를 그대로 사용해도 같은 문제가 발생하지 않습니다.
`window.require`가 없는 브라우저 환경에서는 표준 동작을 그대로 유지합니다.
